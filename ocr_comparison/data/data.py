...
class PrescriptionPillData(Dataset):
    def __init__(self, json_files, mode, args):
        ...
        if getattr(args, 'use_ocr', False):
            ...
            self.text_sentences_tokenizer = None
    ...
    def create_graph(self, bboxes, imgw, imgh, pills_class):
        ...
        for src_idx, src_row in enumerate(bboxes):
            ...
            if src_row['label'] == 'drugname':
                if 'mapping' in src_row:
                    map_str = str(src_row['mapping'])
                    if map_str not in pills_class:
                        pills_label = torch.tensor(-1, dtype=torch.long)
                    else:
                        pills_label = torch.tensor(
                            self.all_pill_labels[map_str], dtype=torch.long)
                else:
                    pills_label = torch.tensor(-1, dtype=torch.long)
            else:
                pills_label = torch.tensor(-1, dtype=torch.long)
            ...
            G.add_node(
                src_idx,
                box_id=src_row['id'],
                text=src_row.get('paddleocr_text', 'unknown') if getattr(self.args, 'use_paddleocr', False) else src_row.get('text', ''),
                bbox=src_row['bbox'],
                y=src_row['y'],
    ...
    def __getitem__(self, idx):
        ...
        if getattr(self.args, 'use_ocr', False):
            text_sentences = self.text_sentences_tokenizer(
                data.text, max_length=32, padding='max_length', truncation=True, return_tensors='pt')
            data.text_sentences_ids = text_sentences.input_ids
            data.text_sentences_mask = text_sentences.attention_mask
        else:
            from PIL import Image
            box_images = []
            for src_idx in range(data.num_nodes):
                box_id = data.box_id[src_idx].item()
                crop_path = osp.join(self.args.data_folder, "prescription_crops", self.mode, pills_image_folder_name, f"{box_id}.jpg")
                if not osp.exists(crop_path):
                    img = Image.new('RGB', (224, 224))
                else:
                    img = Image.open(crop_path).convert('RGB')
                img_tensor = self.transforms(img)
                box_images.append(img_tensor)
                
            data.pres_box_images = torch.stack(box_images)
