...
class ContrastiveLoss(nn.Module):
    ...
    def forward(self, anchor: torch.Tensor, positive: torch.Tensor, negative: torch.Tensor) -> torch.Tensor:
        ...
        loss_contrastive = torch.tensor(0.0, device=anchor.device)
        if len(distance_negative) > 0:
            loss_contrastive = loss_contrastive + torch.mean(torch.pow(distance_negative, 2))
        if len(distance_positive) > 0:
            loss_contrastive = loss_contrastive + torch.mean(torch.pow(torch.relu(self.margin - distance_positive), 2))
